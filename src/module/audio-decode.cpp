...
DecodeAudioResult decode_audio(const std::string &path, float start = 0, float duration = -1, DecodeAudioOptions options = {})
{
  ...
  int64_t start_timestamp = av_rescale_q(start * AV_TIME_BASE, AV_TIME_BASE_Q, stream->time_base);
  avformat_seek_file(format, audio_stream_index, INT64_MIN, start_timestamp, INT64_MAX, 0);

  // Discard packets until we reach the start time
  AVPacket *packet = av_packet_alloc();
  while (av_read_frame(format, packet) >= 0)
  {
    if (packet->stream_index == audio_stream_index)
    {
      double packet_time = packet->pts * av_q2d(stream->time_base);
      if (packet_time >= start)
      {
        break;
      }
    }
  ...
  while (true)
  {
    ...
    while ((status.status = avcodec_receive_frame(codec, frame)) >= 0)
    {
      ...
      if (duration > 0 && current_time - start >= duration)
      {
        break;
      }
    }
